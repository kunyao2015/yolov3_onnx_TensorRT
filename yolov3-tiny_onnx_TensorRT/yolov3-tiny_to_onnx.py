...
class MajorNodeSpecs(object):# 节点
...
class GraphBuilderONNX(object):
    ...
    def _make_input_tensor(self, layer_name, layer_dict):# Net层的解析 layer_name:000_net
        ...
        input_tensor = helper.make_tensor_value_info(  # 输入数据的维度设定
...
def main():
    ...
    #layer_configs = parser.parse_cfg_file(bytes(cfg_file_path,encoding='utf-8'))
    #print(layer_configs)
    ...
    output_tensor_dims['016_convolutional'] = [18, int(img_size/32), int(img_size/32)] # 608/32
    output_tensor_dims['023_convolutional'] = [18, int(img_size/16), int(img_size/16)] # 608/16
